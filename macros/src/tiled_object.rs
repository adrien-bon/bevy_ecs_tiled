...
pub fn expand_tiled_objects_derive(input: syn::DeriveInput) -> proc_macro::TokenStream {
    ...
    let ctor = {
        ...
        for field in fields.iter() {
            ...
            if let Some(attr) = name {
                fields_cton.push(expand_object_fields_rename(&field_name, &attr.meta));
            } else {
                fields_cton.push(expand_object_fields(&field_name));
            }
        }
    ...
    quote::quote! {
        impl bevy_ecs_tiled::properties::traits::TiledObject for #ty {
            fn initialize(
                commands: &mut Commands,
                create_event: &TiledObjectCreated,
            ) {
...
fn expand_object_fields(field_name: &syn::Ident) -> proc_macro2::TokenStream {
    quote::quote!(
        #field_name: if create_event.object_data.properties.contains_key(stringify!(#field_name)) {
            create_event.object_data.properties
...
fn expand_object_fields_rename(
    ...
    quote::quote!(
        #field_name: if create_event.object_data.properties.contains_key(#name) {
            create_event.object_data.properties
                .get(#name)
                .unwrap()
                .clone()
