...
impl<'a> TiledColliderSource {
    ...
    pub fn layer(&self, map: &'a Map) -> Layer<'a> {
        match self {
            TiledColliderSource::Tile { layer_id, x: _, y: _ } => {
                map.get_layer(*layer_id).unwrap()
            }
            TiledColliderSource::Object { layer_id, object_id: _ } => {
                map.get_layer(*layer_id).unwrap()
            }
        }
    }
...
pub(super) fn spawn_collider<T: super::TiledPhysicsBackend>(
    ...
    if let Some((position, entity)) = backend.spawn_collider(commands, map, collider_source, object_data) {
